...
public static void main(String[] args) {
	...
	S.Sim4_Energy_conception_withFire_withFramePeriods(   
			                            association_depth_Energy_Trust_Node // our approach 
			                                            ...
			                                            3600,
			                                            S.PacketTime, S.AckTime, S.Retransmission); // every hour
...
while (t<SimTime){	
 ...
 void Sim4_Energy_conception_withFire_withFramePeriods (int type, int NbreX, int NbreY, double SimTime, double PeriodReAssociation, double PacketTime, double AckTime, int Retransmission)
 {
	...
	//Fp.Change_Node_State( zn); // state...........
	double Delta_R=1;
	double DT=60; // step of simulation fire
...
while (t<SimTime /*&& NbStop<NbMaxStop*/){	
	...
	if (Fp.R<400)	Fp.FireProgress (Fp.R+=Delta_R);/////////////
...
MinEnergy=MidE(zn);
System.out.println("moy "+(EnergyInit-MinEnergy));

MinEnergy=MinE_sup0(zn);

System.out.println("Max "+(EnergyInit-MinEnergy));
